  ...
  <section class="flex flex-col justify-center items-center min-h-screen px-6 py-16 bg-gradient-to-b from-indigo-50 via-white to-pink-50">
    <div class="card max-w-md w-full relative">
      ...
      <h2 class="text-2xl font-bold font-[Nunito] text-center mb-2">
        Now, where does <span class="font-semibold text-violet-700">{{ name }}</span> live? 🏡
      </h2>
      ...
      <button :disabled="!loc" @click="() => $emit('next', { location: loc })" class="w-full text-center bg-violet-600 hover:bg-violet-500 text-white font-bold py-4 rounded-lg text-lg shadow">Continue</button>
